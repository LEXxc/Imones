...
class Company {
    ...
    public function allCompanies(){
        $stmt = $this->pdo->prepare('SELECT * FROM imones');
    ...
    public function editCompany($id, $formData){
        try {
            $query = "UPDATE `imones` SET `pavadinimas` = :name, `kodas` = :code, `pvm_kodas` = :pvm_code, `adresas` = :adress, `telefonas` = :phone, `elpastas` = :email, `veikla` = :activity, `vadovas` = :owner WHERE `id` = :id";
            $stmt = $this->pdo->prepare($query);
